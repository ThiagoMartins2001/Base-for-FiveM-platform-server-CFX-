...
local webhookItem = ""
local webhookclearinv = ""
local webhookblips = ""
local webhookanunciar = ""
local webhookgod = ""
local webhooknc = ""
local webhookkick = ""
local webhooktpway = ""
...
local webhookannouncepolice = ""
local webhookitemall = ""
local webhookaddcar = ""
local webhookbanimentos = ""
local webhookdesbanimentos = ""
local webhooktpcds = ""
local webhooktptome = ""
local webhooktpto = ""
local webhookspectate = ""
local webhookgiveitem = ""
local webhookvisto = ""
...
-- Função auxiliar para verificar se arquivo/diretório existe
...
CreateThread(function()
	...
	local logDir = GetResourcePath(GetCurrentResourceName()).."/logs"
	if not file_exists(logDir) then
		os.execute("mkdir \""..logDir.."\"")
	end
	
	local logFile = logDir.."/playerControl.lua"
	vRP.Archive(logFile,'Data/Horário: '..os.date('%x'..' %X'))
	vRP.Archive(logFile,'Servidor Reiniciado\n\n')

	while true do
		Wait(60*1000*10)
		vRP.Archive(logFile,'Data/Horário: '..os.date('%x'..' %X'))
		vRP.Archive(logFile,'Players Online: '..GetNumPlayerIndices()..'\n')
	end
end)
