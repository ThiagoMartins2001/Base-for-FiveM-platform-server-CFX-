...
AddEventHandler("skinshop:Apply",function(Table)
	-- Verificar se é um preset completo (tem todos os itens necessários)
	local isPreset = true
	...
	-- Se for um preset completo, usar diretamente sem mesclar
	if isPreset then
		Dataset = Table
		print("^2[SKINSHOP] ^7Aplicando preset completo")
	else
		-- Se não for preset, mesclar com Dataset (comportamento original)
		for Index,v in pairs(Dataset) do
			if not Table[Index] then
				Table[Index] = v
			end
		...
		Dataset = Table
		print("^2[SKINSHOP] ^7Aplicando roupas parciais")
	end
	
	vSERVER.Update(Dataset)
	-- Aplicar as roupas diretamente
