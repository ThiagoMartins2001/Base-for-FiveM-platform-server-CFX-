...
function Creative.removeVehicle()
	print("^3[DEBUG REMOVEVEHICLE]^7 Função chamada")
	if not inBennys then
		local Ped = PlayerPedId()
		print("^3[DEBUG REMOVEVEHICLE]^7 Ped: " .. tostring(Ped) .. " | InVehicle: " .. tostring(IsPedInAnyVehicle(Ped)))
		if IsPedInAnyVehicle(Ped) then
			local Vehicle = GetVehiclePedIsUsing(Ped)
			print("^3[DEBUG REMOVEVEHICLE]^7 Vehicle: " .. tostring(Vehicle))
			TaskLeaveVehicle(Ped,Vehicle,0)
			print("^3[DEBUG REMOVEVEHICLE]^7 TaskLeaveVehicle executado")
		end
	end
end
------------------------------------------------------------------------------------------------------------------------------------
...
function Creative.putVehicle(Network)
	print("^3[DEBUG PUTVEHICLE]^7 Função chamada com Network: " .. tostring(Network))
	if NetworkDoesNetworkIdExist(Network) then
		print("^3[DEBUG PUTVEHICLE]^7 Network ID existe")
		local Vehicle = NetToEnt(Network)
		print("^3[DEBUG PUTVEHICLE]^7 Vehicle: " .. tostring(Vehicle))
		if DoesEntityExist(Vehicle) then
			print("^3[DEBUG PUTVEHICLE]^7 Vehicle existe")
			local vehSeats = 10
			local Ped = PlayerPedId()
			print("^3[DEBUG PUTVEHICLE]^7 Ped: " .. tostring(Ped))

			repeat
				vehSeats = vehSeats - 1
				print("^3[DEBUG PUTVEHICLE]^7 Testando assento: " .. vehSeats)

				if IsVehicleSeatFree(Vehicle,vehSeats) then
					print("^3[DEBUG PUTVEHICLE]^7 Assento livre encontrado: " .. vehSeats)
					ClearPedTasks(Ped)
					ClearPedSecondaryTask(Ped)
					SetPedIntoVehicle(Ped,Vehicle,vehSeats)
					print("^3[DEBUG PUTVEHICLE]^7 Jogador colocado no veículo")
			...
			if vehSeats == 0 then
				print("^1[DEBUG PUTVEHICLE]^7 Nenhum assento livre encontrado!")
			end
		else
			print("^1[DEBUG PUTVEHICLE]^7 Vehicle não existe!")
		end
	else
		print("^1[DEBUG PUTVEHICLE]^7 Network ID não existe!")
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- CRUISER
