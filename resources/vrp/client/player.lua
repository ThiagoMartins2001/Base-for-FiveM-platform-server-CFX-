...
function tvRP.ModelExist(Hash)
	...
	if not Hash or type(Hash) ~= "number" then
		print("^1[ERROR] ModelExist: Hash inválido ou não fornecido^7")
		return false
	end
...
function tvRP.Skin(Hash)
	-- Verifica se o Hash é válido
	if not Hash then
		print("^1[ERROR] Skin: Hash inválido ou não fornecido^7")
		return false
	end

	-- Se for string, converte para hash
	if type(Hash) == "string" then
		Hash = GetHashKey(Hash)
	end

	-- Verifica se o hash é um número válido
	...
	-- Verifica se o hash é válido antes de tentar carregar
	if Hash == 0 or Hash == -1 then
		print("^1[ERROR] Skin: Hash inválido após conversão^7")
		return false
	end
