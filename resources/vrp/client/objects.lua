	...
	while true do
		...
		for Number,v in pairs(Objects) do
			if #(Coords - vec3(v["x"],v["y"],v["z"])) <= v["Distance"] then
				if not Init[Number] and v["object"] and LoadModel(v["object"]) then
					if v["mode"] then
...
function tvRP.objectCoords(Model)
	...
	if not Model then
		print("^1[ERROR] objectCoords: Model inválido ou não fornecido^7")
	...
	-- Verifica se o model é um número válido
