...
function vRP.Archive(Archive, Text)
	...
	if not Archive or not Text then
		print("^1[ERROR] Archive: Parâmetros inválidos^7")
		return false
	end
	
	-- Tenta criar o diretório se não existir
	local dir = string.match(Archive, "(.*[/\\])")
	if dir then
		os.execute("mkdir \""..dir.."\" 2>nul")
	end
	
	-- Verifica se o arquivo foi aberto com sucesso
	local file = io.open(Archive, "a")
	if file then
		file:write(Text .. "\n")
		file:close()
		return true
	else
		print("^1[ERROR] Archive: Não foi possível abrir o arquivo " .. tostring(Archive) .. "^7")
		print("^3[INFO] Archive: Verifique se o diretório existe e tem permissões de escrita^7")
		return false
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- Sistema de autenticação removido - Servidor funcionando sem verificação de licença
print('[vRP] Sistema de autenticação removido - Servidor funcionando normalmente.')